...
Wf = 1.0*dana.gaussian((2*N+1,2*N+1), 0.1) - 0.5*dana.gaussian((2*N+1,2*N+1), 1)
focus.connect (focus.V, Wf, 'L', shared=True)
focus.dV = 'minimum(maximum(V+dt/tau*(-V+(L/(N*N)*40*40+I+h)/alpha),0),1)'
plt.figure(figsize=(12,6))
plt.subplot(121)
imshow(input.V, cmap=plt.cm.hot, vmin=0, vmax=1, interpolation='bilinear')
plt.subplot(122)
imshow(focus.V, cmap=plt.cm.hot, vmin=0, vmax=1, interpolation='bilinear')
...
def update(dt):
    ...
    input.V  = dana.gaussian((N,N), 0.2, (x,y))
    x,y = r*np.sin(t+2*np.pi/3), r*np.cos(t+2*np.pi/3)
    input.V  += dana.gaussian((N,N), 0.2, (x,y))
    x,y = r*np.sin(t+4*np.pi/3), r*np.cos(t+4*np.pi/3)
    input.V  += dana.gaussian((N,N), 0.2, (x,y))
    input.V += (2*np.random.random((N,N))-1)*.25
    for image,axis,alpha in items:
        image.update()
